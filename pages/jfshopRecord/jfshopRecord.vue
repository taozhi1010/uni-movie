					...
					<view class="data-item-child">
						<view class="data-item-child-left">
							...
							<view class="child-data">
								<view class="type-name">商品名称：{{item.name}}</view>
								<view class="type-time">{{item.createTime}}</view>
							</view>
						</view>
						<view class="data-item-child-right">
							<!-- <view class="value" v-if="item.moneyComm >= 0">+{{item.moneyComm}}</view> -->
							<view class="value-z">-{{item.moneyComm}}积分</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			withdrawal(down){
				this.api.commUserList(this.page * this.limit, this.limit).then(res => {
			...
			pajectMoneyLog(){
				...
				this.api.commUserList(form).then(res => {
					console.log(res.data)
					if(res.code === "0"){
						this.data = res.data
					}else{
