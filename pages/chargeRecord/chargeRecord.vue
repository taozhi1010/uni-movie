					...
					<view class="data-item-child">
						<view class="data-item-child-left">
							...
							<view class="child-data">
								<view class="type-name">{{formatType(item.type)}}</view>
								<view class="type-time">{{item.cteateTime}}</view>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			withdrawal(down){
				this.api.rechargeRecord(this.page * this.limit, this.limit).then(res => {
