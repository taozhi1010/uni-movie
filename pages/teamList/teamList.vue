<template>
	<s-pull-scroll class="container" 
		ref="pullScroll" :pullDown="pullDown"
...
<!-- 		<mescroll-uni ref="mescrollRef" @down="downCallback" @up="upCallback">
		...
		<view class="flow-infos">
			<view class="flow-info">
				<view class="flow-info-top">{{res.teamList}}</view>
				<view class="flow-info-bottom">团队人数</view>
			</view>
			<view class="flow-info">
				<view class="flow-info-top">{{res.tixian}}</view>
				<view class="flow-info-bottom">总提现</view>
			</view>
			<view class="flow-info">
				<view class="flow-info-top">{{res.counttouzi}}</view>
				<view class="flow-info-bottom">总充值</view>
			</view>
		</view>
				...
				<view class="data-item" v-for="(item, index) in data" :key="index">
					...
					<view class="data-item-child">
						<view class="data-item-child-left">
							...
							<view class="child-data" style="flex: 1;">
								<!-- <view class="type-name">{{item.name}}</view> -->
								<div style="display: flex;justify-content: space-between;">
									<view class="type-time">投资金额：{{item.touziMoney}}</view>
									<view class="type-time">提现金额：{{item.tixian}}</view>
								</div>
								<div style="display: flex;justify-content: space-between;">
									<view class="type-time">收益金额：{{item.shouYiMoney}}</view>
									<view class="type-time">充值金额：{{item.chongzhi}}</view>
								</div>
								
								<div style="display: flex;justify-content: space-between;">
									<view class="type-time">上级名称：{{item.superiorName}}</view>
									<view class="type-time">会员名称：{{item.username}}</view>
								</div>
								<div style="display: flex;justify-content: space-between;">
							<!-- 	<view class="type-time">会员类型：{{item.qqwx}}</view> -->
								<view class="type-time">联系方式：{{item.qqwx}}</view>
								</div>
						...
						<!-- <view class="data-item-child-right"> -->
							...
							<!-- 0未审核  1审核通过，2审核失败 -->
							<!-- <view style="color: orange;">{{formatStatus(item.status)}}</view> -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			withdrawal(down){
				this.api.teamList(this.page * this.limit, this.limit).then(res => {
...
.type-time{font-size: 20upx;color: #6c6c6c;margin-top: 5rpx;flex: 1;}
